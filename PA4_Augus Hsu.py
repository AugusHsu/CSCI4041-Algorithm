...
def max_heapify(task_heap, i):
    ...
    if l <= len(task_heap) and task_heap[l-1].priority > task_heap[i-1].priority:
        print(task_heap[i-1],task_heap[l-1])
        largest = l
    else:
        largest = i
    if r <= len(task_heap) and task_heap[r-1].priority > task_heap[largest-1].priority:
        print(task_heap[i-1],task_heap[r-1])
        largest = r
    if largest != i:
        temp = task_heap[i-1]
        task_heap[i-1] = task_heap[largest-1]
        task_heap[largest-1] = temp
        print(task_heap[i-1])
...
def increase_task_priority(task_heap,i,new_priority):
    i = i +1
    print("this is the task:", task_heap)
    if new_priority < task_heap[i-1].priority:
        return
    task_heap[i-1].priority = new_priority
    while i > 1 and task_heap[math.floor(i/2)-1].priority < task_heap[i-1].priority:
        temp = task_heap[i-1]
        task_heap[i-1] = task_heap[math.floor(i/2)-1]
        task_heap[math.floor(i/2)-1] = temp
        i = math.floor(i/2)
