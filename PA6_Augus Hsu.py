...
def database_insert(database,robot):
    #name
    index_name = hash_function(robot.name)
    database.name_table[index_name].append(robot)
    #shape
    index_shape = hash_function(robot.shape)
    database.shape_table[index_shape].append(robot)
    #color
    index_color = hash_function(robot.color)
    database.color_table[index_color].append(robot)
...
def database_search(database,field,value):
    if field == "name":
        index_name = hash_function(value)
        slot_length = len(database.name_table[index_name])
        out = []
        for i in range(slot_length):
            if database.name_table[index_name][i].name == value:
                out.append(database.name_table[index_name][i])
        return out
    elif field == "shape":
        index_shape = hash_function(value)
        slot_length = len(database.shape_table[index_shape])
        out = []
        for i in range(slot_length):
            if database.shape_table[index_shape][i].shape == value:
                out.append(database.shape_table[index_shape][i])
        return out
    
    elif field == "color":
        index_color = hash_function(value)
        slot_length = len(database.color_table[index_color])
        out = []
        for i in range(slot_length):
            if database.color_table[index_color][i].color == value:
                out.append(database.color_table[index_color][i])
        return out
    
def search_and_delete(table, robot, field):

    if field == "name":

        for i in range(5):
            temp_index = []
            slot_length = len(table[i])
            for j in range(slot_length):
                if table[i][j].name == robot.name:
                    temp_index.append(j)
            table_i_index = [i for i in range(len(table[i]))]
            table_i_index = set(table_i_index)
            temp_index = set(temp_index)
            index = list(table_i_index-temp_index)
            table[i] = [table[i][k] for k in index]      
        ...
        for i in range(5):  
            ...
            slot_length = len(table[i])
            for j in range(slot_length):
                if table[i][j].shape == robot.shape:
                    temp_index.append(j)
            table_i_index = [i for i in range(len(table[i]))]
            table_i_index = set(table_i_index)
            temp_index = set(temp_index)
        ...
        for i in range(5):
            ...
            slot_length = len(table[i])
            for j in range(slot_length):
                if table[i][j].color == robot.color:
                    temp_index.append(j)
            table_i_index = [i for i in range(len(table[i]))]
            table_i_index = set(table_i_index)
            temp_index = set(temp_index)
    ...
    if field == "name":
        ...
        for i in range(slot_length):
            ...
            for index in temp_index:
                temp_robot = database.name_table[index_name][index]
                search_and_delete(database.shape_table,temp_robot,field)
                search_and_delete(database.color_table,temp_robot,field)
        table_index = [i for i in range(len(database.name_table[index_name]))]
        table_index = set(table_index)
        temp_index = set(temp_index)
        index = list(table_index-temp_index)
        database.name_table[index_name] = [database.name_table[index_name][k] for k in index]  
        ...
        for i in range(slot_length):
            if database.shape_table[index_shape][i].shape == value:
                temp_index.append(i)
        for index in temp_index:
                temp_robot = database.shape_table[index_shape][index]
                search_and_delete(database.name_table,temp_robot,field)
                search_and_delete(database.color_table,temp_robot,field) 
        table_index = [i for i in range(len(database.shape_table[index_shape]))]
        table_index = set(table_index)
        temp_index = set(temp_index)
        index = list(table_index-temp_index)
        database.shape_table[index_shape] = [database.shape_table[index_shape][k] for k in index]  
        ...
        for i in range(slot_length):
            if database.color_table[index_color][i].color == value:
                temp_index.append(i)
        for index in temp_index:
                temp_robot = database.color_table[index_color][index]  
                search_and_delete(database.name_table,temp_robot,field)
                search_and_delete(database.shape_table,temp_robot,field)
        table_index = [i for i in range(len(database.color_table[index_color]))]
        table_index = set(table_index)
        temp_index = set(temp_index)
        index = list(table_index-temp_index)
        database.color_table[index_color] = [database.color_table[index_color][k] for k in index]  
