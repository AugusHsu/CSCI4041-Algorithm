...
def insert_floor(floor_tree,floor):
    print("floor_tree",floor_tree.root)
    y = None
    x = floor_tree.root
    while x != None:
        y = x
...
def search(x,k):   
    if k == x.floor_num:
        return x
    if k < x.floor_num:
        if x.left == None and x.parent == None:
            return x
        if x.left == None and x.parent != None:
            temp1 = x.floor_num - k
            temp2 = k - x.parent.floor_num
        ...
        if x.right == None and x.parent == None:
            return x
        
        if x.right == None and x.parent != None:
            temp1 = k - x.floor_num
            temp2 = x.parent.floor_num - k
            if abs(temp1) < abs(temp2):
                return x
            else:
                return x.parent
