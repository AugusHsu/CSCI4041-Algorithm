"""
The template includes a Email class, which consists of two instance variables:
sender, which is a string representing the name of the person or entity who 
sent the email, and subject, which is a string representing the subject line
of the email.  The file also includes some test cases representing sample
inboxes for Holistic Synergies, Ltd. employees.  You’ll need to implement Merge
Sort and Quicksort algorithms which operate on a list of Email objects, and
sort them in non-increasing order by the length of the sender string (that is,
longest sender name to shortest).
"""
...
def merge(A,p,q,r):
    ...
    for k in range(p, r+1):
        ...
        if len(L[i].sender) >= len(R[j].sender):
            #print(len(L[i].sender),len(R[j].sender))
            A[k] = L[i]
            i = i + 1
        else:
            A[k] = R[j]
