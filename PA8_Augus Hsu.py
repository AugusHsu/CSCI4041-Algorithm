...
def insert_point(kd_tree,point):
	if kd_tree.root==None:
		...
		root=kd_tree.root
		while root:
			if (root.depth%2==0 and root.x>=point.x) or (root.depth%2==1 and root.y>=point.y):
				if root.left==None:
					root.left=point
					point.parent=kd_tree
					point.depth=root.depth+1;
					break;
				else:
					root=root.left
			else:
...
def nearest_neighbor(kd_tree,point):
    return nearest_neighbor2(kd_tree.root,point,None)

def nearest_neighbor2(root,point,ans):
	if root ==None:
		return None
	if (root.depth%2==0 and root.x>= point.x) or (root.depth%2==1 and root.y>= point.y):
		if root.left==None:
			if ans==None or point.dist(root)<point.dist(ans):
				ans= root
		else:
			ans=nearest_neighbor2(root.left,point,ans);
		if (root.depth%2==0 and abs(root.x-point.x)<=root.dist(ans))  or (root.depth%2==1 and abs(root.y-point.y)<=root.dist(ans)):
			if root.right==None:
				if ans==None or point.dist(root)<point.dist(ans):
					ans=root
			else:
				ans=nearest_neighbor2(root.right,point,ans);
		return ans;
	else:
		if root.right==None:
			if ans==None or point.dist(root)<point.dist(ans):
